...
public class ComprDataIO {
    ...
    public ComprDataIO(Archive archive2) {
    ...
    public int unpRead(byte[] bArr, int i, int i2) throws IOException, RarException {
        ...
        while (i2 > 0) {
            ...
            if (i4 >= 0) {
                ...
                Volume nextArchive = volumeManager.nextArchive(archive2, archive2.getVolume());
                if (nextArchive == null) {
                    this.nextVolumeMissing = true;
                    return -1;
                }
                FileHeader subHeader = getSubHeader();
                if (subHeader.getUnpVersion() < 20 || subHeader.getFileCRC() == -1 || getPackedCRC() == ((long) (subHeader.getFileCRC() ^ -1))) {
